...
async function main() {
  ...
  const logRes = await fetch(`${supabaseUrl}/rest/v1/database_log?sku=eq.BOOK-NB-666&order=created_at.desc`, {
    headers: { 'apikey': supabaseKey, 'Authorization': `Bearer ${supabaseKey}` }
  });
  const logs = await logRes.json();
  console.log(`=== DATABASE_LOG FOR BOOK-NB-666 (${logs.length} logs) ===`);

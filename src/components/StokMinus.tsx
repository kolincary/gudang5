...
interface ResolvedItemMeta {
    status: string; // 'BARCODE_USED'
    label: string;  // 'Barcode Sudah Dipakai (Tidak Perlu Potong)'
    marked_at: string;
    marked_by?: string;
}
...
const SETTINGS_KEY = 'minus_stock_resolved_items';
...
export const StokMinus: React.FC = () => {
    ...
    const fetchResolvedRecords = async () => {
        let localMap: Record<string, ResolvedItemMeta> = {};
        try {
            const stored = localStorage.getItem(SETTINGS_KEY);
            if (stored) localMap = JSON.parse(stored);
        } catch (e) {
            console.warn('Error reading local resolved minus records:', e);
        }
        ...
        try {
            ...
            if (!error && data && data.value) {
                const dbMap = typeof data.value === 'string' ? JSON.parse(data.value) : data.value;
                const merged = { ...localMap, ...dbMap };
                localStorage.setItem(SETTINGS_KEY, JSON.stringify(merged));
                setResolvedRecords(merged);
                return merged;
            }
        } catch (err) {
            console.warn('Error fetching app_settings minus records:', err);
        }

        setResolvedRecords(localMap);
        return localMap;
    };

    // Save resolved records to app_settings & localStorage
    const saveResolvedRecords = async (records: Record<string, ResolvedItemMeta>) => {
        ...
        try {
            ...
            console.warn('Error saving resolved records to app_settings:', err);
        }
    };
    ...
    const handleToggleMarkResolved = async (row: MinusStockRow) => {
        ...
        const isCurrentlyResolved = Boolean(resolvedRecords[row.id]);
        const nextRecords = { ...resolvedRecords };

        if (isCurrentlyResolved) {
            delete nextRecords[row.id];
            await saveResolvedRecords(nextRecords);
            showToast(`Penandaan dibatalkan untuk ${row.nama_produk}`, 'info');
        } else {
            nextRecords[row.id] = {
                status: 'BARCODE_USED',
    ...
    const handleSendToDatabase = async (row: MinusStockRow) => {
        ...
        try {
            ...
            // Cleanup resolved status if existed
            if (resolvedRecords[row.id]) {
                const nextRecords = { ...resolvedRecords };
                delete nextRecords[row.id];
                saveResolvedRecords(nextRecords);
            }
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            // Cleanup resolved status if existed
            if (resolvedRecords[deleteConfirm.rowId]) {
                const nextRecords = { ...resolvedRecords };
                delete nextRecords[deleteConfirm.rowId];
                saveResolvedRecords(nextRecords);
            }
    ...
    // Filter by search term & status tab
    useEffect(() => {
        let result = rows;

        // Status tab filtering
        if (statusTab === 'PENDING') {
            result = result.filter(r => !resolvedRecords[r.id]);
        } else if (statusTab === 'RESOLVED') {
            result = result.filter(r => Boolean(resolvedRecords[r.id]));
        }
        ...
        if (searchTerm.trim() !== '') {
            ...
            result = result.filter(row =>
                row.nama_produk.toLowerCase().includes(term) ||
                row.rak.toLowerCase().includes(term) ||
    ...
    }, [rows, searchTerm, statusTab, resolvedRecords]);

    // Counts
    const counts = useMemo(() => {
        const resolvedCount = rows.filter(r => Boolean(resolvedRecords[r.id])).length;
        const pendingCount = rows.length - resolvedCount;
        return { total: rows.length, pending: pendingCount, resolved: resolvedCount };
                    ...
                    <div className="bg-gradient-to-br from-red-700 via-rose-800 to-slate-900 pt-[90px] lg:pt-0 lg:h-[310px] pb-[75px] lg:pb-0 px-6 lg:px-12 rounded-b-[40px] lg:rounded-b-[55px] shadow-2xl shadow-red-900/40 relative overflow-hidden transition-all duration-500 flex flex-col justify-center">
                        ...
                        <div className="relative z-10 w-full flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 uppercase text-left">
                            <div className="max-w-2xl">
                                ...
                                <div className="text-red-100/80 font-medium text-[14px] lg:text-[18px] leading-relaxed max-w-[90%] normal-case flex flex-wrap items-center gap-3">
                                    <div className="px-3 py-1 bg-white/10 rounded-full backdrop-blur-sm border border-white/10 flex items-center gap-2">
                                        ...
                                        <span className="text-[11px] font-bold tracking-widest uppercase">{counts.total} Temuan</span>
                                    </div>
                                    {counts.resolved > 0 && (
                                        <div className="px-3 py-1 bg-purple-500/20 rounded-full backdrop-blur-sm border border-purple-400/30 flex items-center gap-1.5 text-purple-200">
                    ...
                    <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center justify-between">
                        ...
                        <div className="relative flex-1 group">
                            ...
                            <input
                                ...
                                className="block w-full pl-12 pr-12 py-3.5 bg-white/70 backdrop-blur-xl border border-white/50 rounded-2xl text-[15px] font-medium shadow-xs transition-all duration-300 focus:ring-4 focus:ring-red-500/5 focus:border-red-500/50 focus:bg-white placeholder:text-slate-400 outline-none"
                            />
                            {searchTerm && (
                                <button
                        ...
                        {/* Status Filter Tabs */}
                        <div className="flex rounded-2xl border border-slate-200/80 p-1 bg-white/80 backdrop-blur-md shadow-xs shrink-0">
                            <button
                                type="button"
                            ...
                            >
                                Barcode Dipakai ({counts.resolved})
                            </button>
                        </div>
                    </div>
                            ...
                            <div className="flex flex-col items-center justify-center py-32 text-slate-400">
                                ...
                                <p className="text-slate-500 text-sm max-w-xs text-center font-medium leading-relaxed">
                                    {searchTerm || statusTab !== 'ALL'
                                        ? 'Kami tidak menemukan data dengan filter / kata kunci tersebut.'
                                        : 'Tidak ada temuan stok minus terdeteksi saat ini.'}
                                </p>
                            </div>
                        ) : (
                                ...
                                <table className="w-full text-sm text-center border-collapse">
                                    <thead>
                                        <tr className="bg-white/50 border-b border-slate-100">
                                            ...
                                            <th className="px-4 py-4 font-black text-slate-400 uppercase tracking-widest text-[10px] text-left">Nama Produk</th>
                                                                ...
                                                                <>
                                                                    {row.total_stok >= 0 && (
                                                                        <Button
                                                                            ...
                                                                            className="h-8 w-8 p-0 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 rounded-lg border border-emerald-200 flex items-center justify-center shadow-2xs"
                                                                    ...
                                                                    <Button
                                                                        ...
                                                                        className="h-8 w-8 p-0 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-lg border border-rose-200 flex items-center justify-center shadow-2xs"
                                                                    ...
                                                                    {isDeveloper && (
                                                                        <Button
                                                                            ...
                                                                            title="Tandai: Tidak Perlu Potong (Barcode Sudah Dipakai)"
                                                                        >
                                    ...
                                    {isResolved ? (
                                        ...
                                        <>
                                            ...
                                            {isDeveloper && (
                                                ...
                                                >
                                                    <Tag className="h-4 w-4" />
                                                    <span className="text-[10px] uppercase font-bold">Tandai</span>
                                                </Button>
                                            )}
                                        </>
                                    )}

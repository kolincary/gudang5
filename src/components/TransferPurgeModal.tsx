...
export const TransferPurgeModal: React.FC<TransferPurgeModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/75 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white border border-rose-200/80 w-full max-w-6xl max-h-[92vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden">
        {/* Top Header */}
        <div className="px-6 py-4 bg-gradient-to-r from-rose-600 via-red-600 to-pink-600 text-white flex items-center justify-between shadow-md">
        ...
        <div className="p-5 border-b border-slate-200 bg-slate-50/50 space-y-4">
          ...
          {/* Protected Racks Banner */}
          <div className="bg-emerald-50/80 border border-emerald-200 p-3 rounded-2xl flex items-start space-x-3 text-xs text-emerald-800 shadow-sm">
            <ShieldCheck className="h-5 w-5 text-emerald-600 flex-shrink-0 mt-0.5" />
            <div className="space-y-0.5">
              <p className="font-bold">Proteksi Rak Khusus Aktif:</p>
              <p className="text-[11px] text-emerald-700 leading-relaxed">
                Log dengan rak <span className="font-mono font-bold text-emerald-900 bg-emerald-100/70 px-1 py-0.5 rounded">{PROTECTED_TRANSFER_RAKS.join(', ')}</span> <strong>100% aman dan tidak akan dihapus</strong>. Sistem otomatis mengecualikannya dari daftar hapus.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-1">
            <div className="bg-white border border-slate-200 p-3 rounded-2xl shadow-sm">
              <span className="text-[10px] font-extrabold uppercase text-slate-400">Total TRANSFER</span>
            ...
            <div className="bg-rose-50 border border-rose-200 p-3 rounded-2xl shadow-sm">
              <span className="text-[10px] font-extrabold uppercase text-rose-700">Siap Dihapus</span>
            ...
            <div className="bg-indigo-50 border border-indigo-200 p-3 rounded-2xl shadow-sm">
              <span className="text-[10px] font-extrabold uppercase text-indigo-700">Dipilih Untuk Dihapus</span>
                    ...
                    {filterTab === 'DELETABLE' && (
                      <th className="p-3 w-10 text-center">
                        <input
                          ...
                          onChange={() => onSelectAllVisible(visibleDeletableIds)}
                        ...
                        <td className="p-3 text-center">
                          {item.isProtected ? (
                            ...
                            <button
                              ...
                              disabled={isDeleting}
                              className="px-2.5 py-1 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-lg font-bold text-[11px] transition-all cursor-pointer inline-flex items-center gap-1"
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
          <div className="text-xs text-slate-500">
            Terpilih: <strong className="text-slate-800">{selectedIds.size}</strong> dari {totalDeletable} baris siap hapus
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={onClose}
          ...
          <div className="bg-white border border-rose-300 w-full max-w-md rounded-2xl shadow-2xl p-6 space-y-4">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button
                ...
                disabled={isDeleting}
                className="px-4 py-2 bg-gradient-to-r from-red-600 to-rose-700 hover:from-red-700 hover:to-rose-800 text-white text-xs font-black rounded-xl shadow-md cursor-pointer flex items-center gap-1.5"
              >
                <Trash2 className="h-3.5 w-3.5" />
                <span>
                  {confirmModalBulk === 'SELECTED'
